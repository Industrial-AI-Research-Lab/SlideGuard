...
def eval_list_criterias(
    ...
    typer.echo("\n📊 Slide-Level Criteria:")
    typer.echo("-" * 25)
    for criteria in registry.get_slide_ids(presentation_type=presentation_type.value):
        typer.echo(f"  • {criteria.value}")
    
    # Deck-level criteria
    typer.echo("\n📋 Deck-Level Criteria:")
    typer.echo("-" * 24)
    for criteria in registry.get_deck_ids(presentation_type=presentation_type.value):
        typer.echo(f"  • {criteria.value}")
    
    slide_total = len(registry.get_slide_ids(presentation_type=presentation_type.value))
    deck_total = len(registry.get_deck_ids(presentation_type=presentation_type.value))
